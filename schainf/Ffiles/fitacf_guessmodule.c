...
/*decl*/
    ...
    PyArrayObject *capi_acf_as_array = NULL;
...
/*frompyobj*/
    ...
    const char * capi_errmess = "fitacf_guess.fitacf_guess.guess: failed to create array from the 1st argument `acf`";
    capi_acf_as_array = ndarray_from_pyobj(  NPY_FLOAT,1,acf_Dims,acf_Rank,  capi_acf_intent,acf_capi,capi_errmess);
    if (capi_acf_as_array == NULL) {
        PyObject* capi_err = PyErr_Occurred();
        if (capi_err == NULL) {
            capi_err = fitacf_guess_error;
            PyErr_SetString(capi_err, capi_errmess);
        }
    } else {
        acf = (float *)(PyArray_DATA(capi_acf_as_array));
    ...
    if (npts_capi == Py_None) npts = shape(acf, 0); else
        f2py_success = int_from_pyobj(&npts,npts_capi,"fitacf_guess.guess() 1st keyword (npts) can't be converted to int");
    if (f2py_success) {
    CHECKSCALAR(shape(acf, 0) == npts,"shape(acf, 0) == npts","1st keyword npts","guess:npts=%d",npts) {
    /* Processing variable tau */
    tau_Dims[0]=npts;
    capi_tau_intent |= F2PY_INTENT_IN;
    const char * capi_errmess = "fitacf_guess.fitacf_guess.guess: failed to create array from the 2nd argument `tau`";
...
/*cleanupfrompyobj*/
    ...
    if((PyObject *)capi_acf_as_array!=acf_capi) {
        Py_XDECREF(capi_acf_as_array); }
    }  /* if (capi_acf_as_array == NULL) ... else of acf */
...
/*frompyobj*/
    ...
    const char * capi_errmess = "fitacf_guess.fitacf_guess.parab1: failed to create array from the 1st argument `x`";
    ...
    const char * capi_errmess = "fitacf_guess.fitacf_guess.parab1: failed to create array from the 2nd argument `y`";
...
static FortranDataDef f2py_routine_defs[] = {
    {"guess",-1,{{-1}},0,0,(char *)  F_FUNC(guess,GUESS),  (f2py_init_func)f2py_rout_fitacf_guess_guess,doc_f2py_rout_fitacf_guess_guess},
    {"parab1",-1,{{-1}},0,0,(char *)  F_FUNC(parab1,PARAB1),  (f2py_init_func)f2py_rout_fitacf_guess_parab1,doc_f2py_rout_fitacf_guess_parab1},
    {"cdtr1",-1,{{-1}},0,0,(char *)  F_WRAPPEDFUNC(cdtr1,CDTR1),  (f2py_init_func)f2py_rout_fitacf_guess_cdtr1,doc_f2py_rout_fitacf_guess_cdtr1},
    {"czte1",-1,{{-1}},0,0,(char *)  F_WRAPPEDFUNC(czte1,CZTE1),  (f2py_init_func)f2py_rout_fitacf_guess_czte1,doc_f2py_rout_fitacf_guess_czte1},
...
PyMODINIT_FUNC PyInit_fitacf_guess(void) {
    ...
    s = PyUnicode_FromString(
        "This module 'fitacf_guess' is auto-generated with f2py (version:2.0.2).\nFunctions:\n"
"    guess(acf,tau,zero,amin,te,tr,npts=shape(acf, 0))\n"
"    parab1(x,y,a,b,c)\n"
"    cdtr1 = cdtr1(depth)\n"
"    czte1 = czte1(zlag,tr)\n"
".");
