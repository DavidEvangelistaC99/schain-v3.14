...
class SpectraAFCProc_V0(ProcessingUnit):
...
#         self.dataOut.nProfiles = self.dataOut.nFFTPoints
    ...
    def run(self, nProfiles=None, nFFTPoints=None, pairsList=[], code=None, nCode=1, nBaud=1,real= None, imag=None):
        ...
        if self.dataIn.type == "Spectra":
            ...
            self.dataOut.data_spc = acf.real
...
#         self.dataOut.nProfiles = self.dataOut.nFFTPoints
    ...
    def run(self, nProfiles=None, nFFTPoints=None, pairsList=[], code=None, nCode=1, nBaud=1):
        ...
        if self.dataIn.type == "Spectra":
            ...
            aux2 = numpy.fft.fft(self.dataOut.data[0,:,hei_id],n = 16)
            aux2 = numpy.fft.fftshift(aux2)
            aux2 = aux2*numpy.conjugate(aux2)
            aux2 = aux2.real #Este valor es el que da SCh
            print("spc 2: ",numpy.sum(aux2))
            aux2        =  numpy.fft.fftshift(aux2)
            aux2        =  numpy.fft.ifft(aux2)
            print("Rate_Right?: ",aux2[0]/corr[0,0,hei_id])

            print("AFC sum: ",numpy.sum(acf[0,:,hei_id]))
            print("aux2 sum: ",numpy.sum(aux2))
            print("Rate: ",acf[0,0,hei_id]/corr[0,0,hei_id])
            print("Rate aux: ",acf[0,0,hei_id]/corr_aux[0,-1,hei_id])
            '''
            #print(acf[0,:,150])
            #exit(1)
            '''
            ...
            for j in range(shape[0]):
                for i in range(shape[2]):
                   ...
                   acf[j,:,i][tmp] = value
                   '''
            #acf        =  numpy.fft.fftshift( acf, axes=(1,))
            '''
            # Normalizando
            for i in range(shape[0]):
                for j in range(shape[2]):
            ...
            #self.dataOut.data_acf = acf[:,:16,:]#*2
            #self.dataOut.data_spc = acf[:,:16,:].real#*2

            self.dataOut.data_acf = acf
            '''
            self.dataOut.data_spc = data.imag

            print("Real: ",data[0,:,26].real)
            print("Real dB:",10*numpy.log10(data[0,:,26].real))
            print("Imag: ",data[0,:,26].imag)
            print("Imag dB:",10*numpy.log10(data[0,:,26].imag))
            exit(1)
            '''
            #print("AFC",self.dataOut.flagNoData)
            #'''
            #print("acf 0: ", self.dataOut.data_acf[0,0,100])
            #print("spc: ",numpy.mean(self.dataOut.data_spc[0,:,100]))
            #print("spc 0: ",numpy.fft.fftshift(self.dataOut.data_spc[0,:,100])[0])
            #exit(1)
            #self.dataOut.data_spc = acf.real
            #print(self.dataOut.data_acf[0,:,0])
            #exit(1)
            ...
            #print(self.dataOut.heightList[hei])
            #plt.plot(z[0,0,:],self.dataOut.heightList)
            aux =  self.dataOut.data_acf[0,:,100]
            power = aux*numpy.conjugate(aux)
            #print(power)
            powerdb = numpy.log10(power)
            #plt.plot(powerdb,self.dataOut.heightList)
            #plt.plot(self.dataOut.data_acf[0,:,33])
            #plt.plot(corr)
            #plt.imshow(corr.real[0])
            plt.imshow(self.dataOut.data_acf.real[0])
            #plt.ylim(0,1000)
            plt.grid()
            plt.show()
            exit(1)
