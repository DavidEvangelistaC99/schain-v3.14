...
PARAM = {
    ...
    'SNR':{'zmin': -10, 'zmax': 30, 'colormap': 'jet'    , 'label': 'SNR', 'wrname': 'snr','cb_label': 'dB', 'ch':0},
...
def main(args):
    ...
    if not conf['usrp_tx']['enable_2'] or (conf['usrp_tx']['enable_2'] and not conf['usrp_tx']['repetitions_2']): # One Pulse
        ...
        op.addParameter(name='frequency', value=9345000)
        ...
        if args.spc:
            spc_proc = project.addProcUnit(datatype='SpectraProc', inputId=voltage2.getId())
            spc_proc.addParameter(name='nFFTPoints', value=500)
            spc_proc.addParameter(name='nProfiles', value=500)
            input_id = spc_proc.getId()
        ...
        if '1' in args.pulses:
            ...
            if args.spc:
                spc_proc1 = project.addProcUnit(datatype='SpectraProc', inputId=voltage1.getId())
                spc_proc1.addParameter(name='nFFTPoints', value=int(conf['usrp_tx']['repetitions_1']/ncode))
                spc_proc1.addParameter(name='nProfiles', value=int(conf['usrp_tx']['repetitions_1']/ncode))

                if args.rmDC:
                    op = spc_proc1.addOperation(name='removeDC', optype='self')
                                
                input_id = spc_proc1.getId()
            else:
                op = voltage1.addOperation(name='PulsePair_vRF', optype='other')
                op.addParameter(name='n', value=int(conf['usrp_tx']['repetitions_1'])/ncode, format='int')
                if args.rmDC:
                    op.addParameter(name='removeDC', value=1, format='int')
                input_id = voltage1.getId()

            proc1 = project.addProcUnit(datatype='ParametersProc', inputId=input_id)
            proc1.addParameter(name='runNextUnit', value=True)

            if args.spc:
                op = proc1.addOperation(name='SpectralMoments', optype='other')
        ...
        if '2' in args.pulses:
            ...
            if args.spc:
                spc_proc2 = project.addProcUnit(datatype='SpectraProc', inputId=voltage2.getId())
                spc_proc2.addParameter(name='nFFTPoints', value=int(conf['usrp_tx']['repetitions_2']/ncode))
                spc_proc2.addParameter(name='nProfiles', value=int(conf['usrp_tx']['repetitions_2']/ncode))
                input_id = spc_proc2.getId()

                if args.rmDC:
                    op = spc_proc2.addOperation(name='removeDC', optype='self')                
                ...
                if args.rmDC:
                    op.addParameter(name='removeDC', value=1, format='int')
                input_id = voltage2.getId()


            proc2 = project.addProcUnit(datatype='ParametersProc', inputId=input_id)
            proc2.addParameter(name='runNextUnit', value=True)

            if args.spc:
                op = proc2.addOperation(name='SpectralMoments', optype='other')
            ...
            opObj10.addParameter(name='tauW',value=(1e-6/sample_rate)*len(code[0]))
            opObj10.addParameter(name='Pt',value=200)        
            opObj10.addParameter(name='mask',value=MASK2)
            opObj10.addParameter(name='variableList',value=parameters)
            opObj10.addParameter(name='noise_angle', value=2.6)
        ...
        xrange = args.range if args.range>0 else 60
        for param in parameters:
            ...
            if args.plot:
                ...
                op.addParameter(name='xrange', value=xrange, format='int')
                op.addParameter(name='attr_data', value=param, format='str')
                op.addParameter(name='labels', value=[[PARAM[param]['label']], [PARAM[param]['label']]])
                op.addParameter(name='save_code', value=param)
                op.addParameter(name='cb_label', value=PARAM[param]['cb_label'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mask', default='4', type=str,
...
# Test TFM
# python3 sophy_proc_tfm.py CHIRP_TFM@2026-01-27T20-36-02 --parameters SNR --plot --save --rmDC --label 27_04_26 --range 60 --mask -9.0
